...
DATA_SECTION
 ...
 imatrix F_y_ini_used(first_VPA,nsp,fyModel,lyModel);
 !! F_y_ini_used=1;
 ...
 // Normalise to mean effort=1   SKAL ÆNDRES TIL NORMALISERES INDENFOR ÅRRÆKKE MED SAMME EXPLOITATION PATTERN (MÅSKE??)
 number sum_effort
 !! if (any_do_effort==1) {
...
//*********************************************************************************************
 ...
 3darray index_maxl(1,def_yqd,1,yqdAll,yqdsAll_fa,yqdsAll_la) // last size class in use
  ...
  4darray size_l_sea(1,def_yqd,1,nsp,fa,max_a,minl,maxl)    //length or weight of species at age in the sea BURDE VÆRE "RAGGED"
...
PARAMETER_SECTION
 ...
 !! if (phase_F_y_ini>0 ) {
 !!   for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==0) for (y=fyModel+1;y<=lly(s);y++) if(F_y_ini_used(s,y)==1){
 !!    parNo++;
 !!    parexp<<"F_y_ini "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1 "<<s<<" "<<y<<" -1 used" <<endl;;
 !!   }
 !! }
 //!! cout<<"F_y_ini_used: "<<F_y_ini_used <<endl;
 ...
 !!     for (syg=1;syg<=n_catch_sep_year_group(s);syg++) n_log_F_a_ini++;
 !!   }
 !!  }
 ...
 !! int n_catch_s2_used=0;
 !! int maa=0;
 !! for (s=first_VPA;s<=nsp;s++) {
 !!   if (seasonal_combined_catch_s2(s)>1) {   // seasonal data
 !!    for (q=1;q<=seasonal_combined_catch_s2(s);q++) {
 !!     for (j=1;j<=n_catch_s2_group(s);j++) {
 !!       if (j<n_catch_s2_group(s)) k=catch_s2_group(s,j+1)-1; else k=la_like(s);
 !!       maa=0;
 !!       for (a=catch_s2_group(s,j); a<=k;a++) {
 !!          if (incl_catch_season_age(s,q,a) >0) maa++;
 !!        }
 !!        if (maa>0) { n_catch_s2_used++;  catch_s2_used(s,q,j)=n_catch_s2_used; }
 !!     }
 !!    }
 !!   }  else for (j=1;j<=n_catch_s2_group(s);j++) {
 !!         n_catch_s2_used++;
 !!         catch_s2_used(s,1,j)=n_catch_s2_used;
 !!      }
 !! }
...
//lower and upper value for stomach variance parameter 
 ...
 //sdreport_matrix avg_FF(1,7,fq,3);
 
 !! k=0;
 !! for (s=first_VPA;s<=nsp;s++) for (y=lly(s)-sdReportYear;y<=lly(s);y++) if (F_y_ini_used(s,y) != -1) k++;

 sdreport_vector avg_F(1,k)
 !! for (s=first_VPA;s<=nsp;s++) for (y=lly(s)-sdReportYear;y<=lly(s);y++) if (F_y_ini_used(s,y) != -1) {
 !!    parNo++;  parexp<<"avg_F "<<parNo<<" "<<s<<" "<<y<<" -1 -1 -1 -1 -1 -1 "<<s<<" "<<y<<" -1 used"<<endl;;
 !! }
 //vector  avg_F(1,k)
...
PRELIMINARY_CALCS_SECTION
 ...
 //stock_N_likeprof.set_stepsize(0.2); // default value is 0.5
  ...
  F_y=1.0;
  for (s=first_VPA;s<=nsp;s++) for (y=fyModel+1;y<=lyModel;y++) if (F_y_ini_used(s,y)== -1) F_y(s,y)=0;
 ...
 if (sum(log_F_a_ini)>0) log_F_a_ini=0.0;
...
FUNCTION void calc_M2_ALK(int y, int q, int d, dvector other_food, d3_array size,d3_array consum, dvar_matrix N_bar, dvar_matrix N, dvar_matrix F, dvar_matrix M1, dvar_matrix& M2);
 ...
 for (prey=first_VPA;prey<=nsp;prey++) if (is_prey(prey)==1) {
   for (prey_a=faq(q);prey_a<=la(prey);prey_a++) {
     ...
     for (prey_l=int(index_minl(yqd,prey,prey_a));prey_l<=int(index_maxl(yqd,prey,prey_a));prey_l++) {
       if (use_Nbar==0) {                    // VRØVL ???? Nej OK  Use N in the mean N to calc dead numbers
...
FUNCTION void calc_F(int do_exploitaion_pattern)
 ...
 for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==0) {
    if (Nknots(s)>1) {   // splines are used
      ...
      // copy F_y_ini to F_y
      for (y=fyModel+1;y<=lyModel;y++) if(F_y_ini_used(s,y)==1) {
        i++;
        F_y(s,y)=F_y_ini(i);
      }
    }
 }
 //cout<<"F_y: "<<F_y<<endl<<endl;

 // Copy F_q. F_q(s,g,lq_F(s,a)) is constant
 //cout<<"F_q_ini: "<<endl; for (i=1;i<=no_sp_sag_syg;i++) {for (j=fcq(i);j<=lcq(i);j++) cout<<F_q_ini(i,j)<<" "; cout<<endl; }
     ...
     for (sag=1;sag<=n_catch_season_age_group(s);sag++) {
      ...
      for (syg=1;syg<=n_catch_sep_year_group(s);syg++) {                
         ...
         // old MV if (seasonal_annual_catches(s)==0 )     { for(q=fq_F(s,a);q<lq_F(s,a);q++) F_q(s,sag,syg,q)=F_q_ini(sp_sag_syg,q);}
          if (seasonal_annual_catches(s)==0 )     {
            for(q=fq_F(s,a);q<lq_F(s,a);q++) {
              if (incl_catch_season_age(s,q,a)==1) {
                F_q(s,sag,syg,q)=F_q_ini(sp_sag_syg,q);
                //cout<<"F_q_ini(sp_sag_syg,q): "<<F_q_ini(sp_sag_syg,q)<<endl;
              }
            }
          }
      }
 ...
 //cout<<"F_q: "<<endl<<endl<<F_q<<endl;


 //cout<<"log_F_a_ini: "<<endl<<log_F_a_ini<<endl;
  // Copy F_a, 
  i=0;
  for (s=first_VPA;s<=nsp;s++){
    for (a=cfa(s);a<=las(s);a++){
      for(syg=1;syg<=n_catch_sep_year_group(s);syg++) {
        ...
        //cout<<"s:"<<s<<" ly_group: "<<ly_group<<"  catch_sep_year(s,syg):"<<catch_sep_year(s,syg)<<" i:"<<i<<endl;
        for (y=catch_sep_year(s,syg);y<=ly_group;y++) {
            F_a(s,y,a)=exp(log_F_a_ini(i));
         }
       }
     }
 ...
 // Calc F, Separable model
  ...
  //cout<<"F(yq,s,a): "<<endl<<F<<endl<<endl;
   ...
   for (s2_group=1;s2_group<=n_catch_s2_group(s);s2_group++) {
     ...
     for (y=fyModel;y<=lyModel;y++) {
        if (seasonal_annual_catches(s)==0) { 
         ...
         else {     // use annual catches in objective function, kønt er det ikke
...
FUNCTION  evaluate_SSB_recruitment_contributions
 ...
 double mins,epsilon;
  ...
  gamma=10; // constant from Benoit Mesnil and Marie-Joëlle Rochet, A continuous hocky stick stock-recruit model for estimating MSY reference points. ICES Journal of Marine Science, 67: June 8, 2010
 ...
 for (s=first_VPA;s<=nsp;s++){
   for (y=fyModel;y<=lyModel;y++){
     ...
     for (a=avg_F_ages(s,1);a<=avg_F_ages(s,2);a++) {
      ...
      for (q=rc;q<=lq;q++){
         CALC_yq 
         meanN=N(yq,s,a)*(1-exp(-Z(yq,s,a)))/Z(yq,s,a);
         if (multi>=1) {
     ...
     if (sd_phase() && (y>=lyModel-sdReportYear)) {
        if (F_y_ini_used(s,y) != -1){ k++; avg_F(k)=Mean_F(s,y);}
     }
   }
 }

 //if (sd_phase()) {for (a=1;a<=7;a++) for (q=fq;q<=3;q++){ y=2020; CALC_yq; avg_FF(a,q)=F(yq,first_VPA,a);} }
